...
## [Unreleased]
- **fix(remote): swipe download benar-benar bersih (2/2)** -- (1) `li.item` kini murni container: `position: relative; overflow: hidden; touch-action: pan-y` — border-radius, box-shadow, transition dan hover transform dihapus total (sebelumnya masih ada sehingga shadow statis dan sudut membulat bocor ke background saat kartu digeser). (2) Latar swipe dibuat solid penuh: `.swipe-bg.left/right` tetap `inset: 0; z-index: 0`, dan JS kini hanya menyalakan SATU background sesuai arah (kiri → amber "Pause", kanan → merah "Delete") tanpa ramp transparan, jadi tidak ada strip putih/abu di area terbuka. (3) `.swipe-content` (z-index 1) memegang semua style kartu: background `--card`, border, border-left status, border-radius, padding. (4) Threshold aksi (-80/+80) dan logika Pause/Resume/Delete tidak diubah.
 -- Pindahkan semua style visual kartu (background, border, border-left status, border-radius, padding) dari `li.item` ke `.swipe-content`. `li.item` kini hanya container (`position: relative; overflow: hidden; border-radius; box-shadow`). Saat `.swipe-content` digeser horizontal, background amber/merah muncul bersih tanpa gap putih. Compact mode dan batch mode tetap bekerja normal. `attachSwipe()` tidak diubah — hanya CSS yang diperbaiki.
