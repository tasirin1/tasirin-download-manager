## [Unreleased]
- **Fix progres HLS/adaptive tidak bergerak** — Progres YouTube HLS sebelumnya selalu `totalBytes = 0` sehingga bar progres diam di 0 padahal byte terunduh terus bertambah. Kini progres HLS memakai estimasi total (rata-rata ukuran segmen x jumlah segmen) agar bar & persentase bergerak; jalur adaptive (video/audio MP4) memakai Content-Length bila tersedia. `progressPercent` murni (bisa diuji) tetap: `(bytes*100)/total`, estimasi total berubah halus saat segmen selesai dan lompat ke 100% saat selesai.
