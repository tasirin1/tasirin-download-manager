## [Unreleased]
- **Fix YouTube CDN 403 (fallback)** — Perbaiki compile error (continue di lambda repeat → for loop). URL googlevideo hasil ekstraksi halaman kini butuh transformasi `n`-signature YouTube (HTTP 403 permanen). Deteksi via probe HEAD: bila 403, fallback berlapis ke instance Piped lalu Invidious (`/latest_version` yang menyelesaikan n-signature di sisi server). Gagal cepat dengan pesan jelas bila semua fallback mati (tidak buang waktu retry). Tambah log `YT DEBUG` untuk tiap langkah.
