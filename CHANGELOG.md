...
## [Unreleased]
- **fix(remote): redesain swipe download sepenuhnya** -- Ganti pendekatan swipe dari background penuh lebar (`swipe-bg inset:0`) menjadi panel aksi lebar tetap 84px (`swipe-action`). Struktur: `.swipe-action.left` (amber, 84px kiri) + `.swipe-action.right` (merah, 84px kanan) + `.swipe-content` (kartu utama). Panel default `display:none` (base rule `.swipe-action`), hapus `display:flex` dari rule `li.item .swipe-action` yang meng-override visibility karena spesifisitas lebih tinggi. JS menampilkan panel (`display:'flex'`) hanya saat gesture swipe aktif dan reset ke `none` saat touchend. Transisi transform di-nonaktifkan saat drag (`transition:none`) lalu diaktifkan kembali agar kartu kembali mulus. Threshold aksi 48px. `li.item` murni container. Artefak putih, strip permanen, dan kebocoran panel hilang.
