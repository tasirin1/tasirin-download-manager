## [Unreleased]
- **Fix ekstraksi foto carousel Instagram** — `extractAllDisplayUrlsFromPage` sebelumnya men-filter semua URL yang mengandung `s640x640` di query parameter, yang justru menghapus semua foto carousel. Rewrite: cari URL `t39.30808-6` (path CDN foto post Instagram), deduplicate per file ID, prefer full-res. Sekarang carousel 2+ foto terdeteksi dengan benar.
