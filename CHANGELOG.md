## [Unreleased]
- **Fix video HLS tidak jalan / lag / desync / ukuran besar** — Timeline video kini deterministik: jumlah frame dihitung dulu (pass terpisah), lalu durasi total dari `#EXTINF` dibagi rata ke tiap frame sehingga timestamp selalu seragam, naik terus, dan sinkron dengan audio. PTS MPEG-TS YouTube tidak dipakai lagi (restart per segmen & span tidak proporsional membuat timestamp duplikat/bergelombang → video tak bisa diputar atau stutter). Fallback PTS-with-drift untuk playlist tanpa EXTINF. Kualitas diturunkan 720p 3.8Mbps → 480p 1.1Mbps (itag 231) agar ukuran lebih kecil dan remux ringan.
