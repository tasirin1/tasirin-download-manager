## [Unreleased]
- **Fix HLS lag/desync/ukuran besar** — Timeline video dibangun dari durasi segmen `#EXTINF` (bukan PTS MPEG-TS yang restart per segmen dan span tidak proporsional) sehingga sinkron dengan audio (frame-count × 1024/sampleRate). Deteksi restart segmen via PTS turun > 1 detik. Fallback PTS-with-drift untuk playlist tanpa EXTINF. Kualitas diturunkan dari 720p 3.8Mbps → 480p 1.1Mbps (itag 231) agar ukuran file lebih kecil dan proses remux lebih ringan.
