...
## [Unreleased]
- **fix(remote): swipe download card tanpa garis putih** -- Pindahkan semua style visual kartu (background, border, border-left status, border-radius, padding) dari `li.item` ke `.swipe-content`. `li.item` kini hanya container (`position: relative; overflow: hidden; border-radius; box-shadow`). Saat `.swipe-content` digeser horizontal, background amber/merah muncul bersih tanpa gap putih. Compact mode dan batch mode tetap bekerja normal. `attachSwipe()` tidak diubah — hanya CSS yang diperbaiki.
