## [Unreleased]
- **Bersihkan semua warning lint (0 errors, 0 warnings)** - Ubah tiga string jumlah ringkasan menu utama (`summary_active`/`summary_done`/`summary_failed`) menjadi `<plurals>` + `getQuantityString` (menghilangkan 3 `PluralsCandidate`), dan ganti `Uri.parse(url)` menjadi ktx `url.toUri()` di `MainActivity.platformLabelFrom` (menghilangkan `UseKtx`).
