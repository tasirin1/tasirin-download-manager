...
## [Unreleased]
- **fix(remote): redesain swipe download sepenuhnya** -- Ganti pendekatan swipe dari background penuh lebar (`swipe-bg inset:0`) menjadi panel aksi lebar tetap 84px (`swipe-action`). Struktur baru: `.swipe-action.left` (amber, 84px kiri) + `.swipe-action.right` (merah, 84px kanan) + `.swipe-content` (kartu utama). Panel hanya muncul di sisi yang relevan (JS toggle `display` tanpa transisi opacity). Transisi transform di-nonaktifkan selama drag (`transition: none`) dan diaktifkan kembali saat touchend agar kartu kembali mulus. Threshold aksi 48px (sebelumnya 80px). `li.item` murni container tanpa visual. Artefak putih, bentuk aneh, dan sobek hilang total.
