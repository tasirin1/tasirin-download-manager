...
- **Hapus Sort dari halaman utama** — Chip `Sort` di baris filter dihapus beserta dialog sortir, `updateSortButton()`, `showSortDialog()`, string `sort_by`/`sort_options`/`label_value`, dan `StoragePrefs.setSortMode`. Urutan daftar tetap mengikuti preferensi sortir tersimpan lewat `applyFilter()`.
