## [Unreleased]
- **Fix YouTube CDN 403 (fallback)** — URL googlevideo hasil ekstraksi halaman kini butuh transformasi `n`-signature YouTube (HTTP 403 permanen). Deteksi via probe HEAD: bila 403, fallback ke instance Piped (kembalikan URL yang sudah di-resolve). Gagal cepat dengan pesan jelas bila semua fallback mati (tidak buang waktu retry). Tambah log `YT DEBUG` untuk tiap langkah.
