## [Unreleased]
- **Fix ekstraksi foto carousel Instagram** — `extractAllDisplayUrlsFromPage` sebelumnya men-filter semua URL yang mengandung `s640x640` di query parameter dan hanya mencocokkan path CDN `t39.30808-6`, yang justru menghapus semua foto carousel (Instagram menggunakan berbagai path CDN: `t51.82787-15`, `t51.82787-19`, `t39.30808-6`). Rewrite: regex sekarang mencocokkan semua URL gambar dari `scontent*.cdninstagram.com`, membuang profil pic (`t51.2885-*`), deduplicate per ID file, dan memilih versi full-res. Sekarang carousel 2+ foto terdeteksi dengan benar.
