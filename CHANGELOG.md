## [Unreleased]
- **Mode Audio-only (M4A) di menu Add Download** — Saat link media sosial (khususnya YouTube) terdeteksi, dialog Add Download kini menampilkan checkbox "Audio only (M4A)". Jika dicentang, download hanya mengambil jalur audio (itag 140/139 AAC) dan menghasilkan file `.m4a` tanpa video dan tanpa remux — hemat bandwidth & ruang. Alur ini dipakai untuk jalur YouTube via HLS maupun hasil ekstraksi media sosial lain yang menyediakan URL audio terpisah; bila audio gagal, fallback otomatis ke video agar user tetap mendapat sesuatu. (`DownloadItem.audioOnly`, `DownloadEngine.downloadAudioOnly`, dialog & `MainActivity` wiring.)
