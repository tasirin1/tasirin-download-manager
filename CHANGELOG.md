...
- **Desain ulang menu utama (paket 1-4)** — (1) Item download berlapis: card memakai bayangan lembut (`bg_card_shadow`), strip status kiri berwarna sesuai state, badge status berbentuk pill berwarna (bukan titik kecil), dan persen tampil besar di kanan. (2) Empty state lebih hidup: ilustrasi lingkaran + ikon download (tint primary), teks lebih jelas, dan 4 chip saran platform (YouTube/TikTok/Instagram/X) yang membuka dialog Add Download dengan URL contoh. (3) Daftar download dikelompokkan per section (Active → Paused → Completed → Failed) lewat baris header di adapter — tanpa menghidupkan filter; swipe di header dilarang. (4) Ringkasan header kini memakai titik berwarna per state (`●` biru/hijau/merah) via SpannableString; drawable lama `bg_card_rounded` & `bg_status_dot` dihapus karena tidak terpakai lagi.
